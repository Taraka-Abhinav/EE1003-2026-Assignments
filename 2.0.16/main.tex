...
\textbf{We need to determine:}

\vspace{0.5em}

\begin{itemize}

\item \textbf{Completion times}
\[
C = \begin{bmatrix}C_1\\C_2\\C_3\\C_4\end{bmatrix}
\]

\item \textbf{Order of jobs}
\[
X_{ij} =
\begin{cases}
1 & \text{if job } i \text{ is before } j \\
0 & \text{otherwise}
\end{cases}
\]

\item \textbf{Maximum tardiness}
\[
T_{\max}
\]
...
\item Use binary variables $X_{ij}$ to represent ordering
\item Enforce \textbf{no overlap} using Big-M constraints
\item Compute completion times automatically
\item Minimize the worst delay ($T_{\max}$)
...
\begin{itemize}
\item The solver tries all feasible sequences implicitly
\item Returns the optimal schedule directly
\end{itemize}
...
\begin{frame}{Sequencing and Tardiness Constraints}

\textbf{No-overlap (Big-M):}
\[
C_i + p_j \le C_j + M(1 - X_{ij}), \quad i \ne j
\]

\vspace{0.5em}

\textbf{Tardiness definition:}
\[
T_{\max} \ge C - d
\]
...
\textbf{Meaning:}
\begin{itemize}
\item Ensures correct job order
\item Captures maximum delay among all jobs
\end{itemize}
...
\textbf{Ordering matrix (interpreted):}

\begin{itemize}
\item $R$ precedes all jobs
\item $P$ precedes $Q, S$
\item $Q$ precedes $S$
\end{itemize}
...
\begin{frame}{Final Schedule}

\textbf{Optimal job sequence:}
\[
...
\begin{itemize}
\item Sequence is obtained directly from optimization
\item No heuristic or rule required
\end{itemize}
...
\textbf{Conclusion:}
\begin{itemize}
\item MILP model guarantees optimal solution
\item CVXPY computes it efficiently
\end{itemize}
...
\begin{frame}[fragile]{CVXPY Implementation}
\begin{lstlisting}
import cvxpy as cp
import numpy as np
...
for i in range(n):
    for j in range(n):
        if i != j:
            constraints += [
                C[i] + p[j] <= C[j] + M*(1 - X[i,j])
            ]

prob = cp.Problem(cp.Minimize(Tmax), constraints)
prob.solve(solver=cp.ECOS_BB)
\end{lstlisting}
\end{frame}

\begin{frame}{CVXPY Solver[ECOS] Output}
    \begin{figure}
        \centering
        \includegraphics[width=0.7\linewidth]{figs/CVXPY.png}
    \end{figure}
\end{frame}
