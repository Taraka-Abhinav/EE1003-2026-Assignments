...
\begin{frame}{\textbf{Advanced: ODE Formulation}}
  \begin{center}
    ...
    \[
      \frac{dV_C(t)}{dt} \;+\; \left( \frac{R_1 + R_2}{R_1 R_2 C} \right) V_C(t) \;=\; \frac{V_s}{R_1 C}
    \]
...
%% ── ADVANCED: DIFFERENTIAL EQUATION SOLVED ───────────────────────
...
\begin{frame}{\textbf{Advanced: Transfer Function $H(s)$}}
  \begin{center}
    \vfill
    Analyzing the circuit as a system mapping $V_s(t) \to V_C(t)$ in the \textbf{$s$-domain}.
    
    \vspace{1.5em}
    The Transfer Function is found via \textbf{voltage division} with impedances:
    \[
      H(s) \;=\; \frac{V_C(s)}{V_s(s)} \;=\; \frac{R_2 \parallel \frac{1}{sC}}{R_1 + \left( R_2 \parallel \frac{1}{sC} \right)}
    \]
    ...
    \vspace{1.5em}
    \normalsize
    \textit{The pole is located at $s = -25\,\mathrm{rad/s}$, which defines the system's cutoff frequency.}
    \vfill
  \end{center}
\end{frame}
...
\begin{frame}{\textbf{Advanced: Laplace Transform}}
  \begin{center}
    \vfill
    In the \textbf{$s$-domain}, the input is a step: $V_s(s) = \frac{10}{s}$.
...
\begin{frame}{\textbf{Advanced: State-Space Formulation}}
  \begin{center}
    ...
    \[
      \mathbf{\dot{x}(t) \;=\; \underbrace{-25}_{\mathbf{A}} \, x(t) \;+\; \underbrace{12.5}_{\mathbf{B}} \, u(t)}
    \]
    
    \vspace{1.5em}
    \normalsize
    \textit{The eigenvalue of $\mathbf{A}$ is $\lambda = -25$. \\ Since $\lambda < 0$, the system is strictly stable.}
    \vfill
  \end{center}
\end{frame}
...
\begin{frame}{\textbf{Advanced: Energy Dynamics}}
  \begin{center}
    ...
    \[
      E_C \;=\; \frac{1}{2} C V_\infty^2 \;=\; \frac{1}{2} (4\,\mu\mathrm{F}) (5\,\mathrm{V})^2 \;=\; \mathbf{50\,\mu\mathrm{J}}
    \]
    ...
    \vspace{1.5em}
    \normalsize
    \textit{Maximum power transfer occurs at $t = \frac{\ln(2)}{25} \approx \mathbf{27.7\,\mathrm{ms}}$.}
    \vfill
  \end{center}
\end{frame}
...
\begin{frame}[fragile]{\textbf{Python Code: Power \& Energy (Themed)}}
\scriptsize
\begin{lstlisting}
import numpy as np
import matplotlib.pyplot as plt
...
ax1.plot(t*1000, power_mW, color=power_color)
ax2 = ax1.twinx()
ax2.plot(t*1000, energy_uJ, color=energy_color)
...
\begin{frame}{\textbf{Plot - Power \& Energy}}
    \begin{figure}
        \centering
        \includegraphics[width=1\linewidth]{figs/PS.png}
        \label{fig:placeholder}
    \end{figure}
\end{frame}

\begin{frame}[fragile]{\textbf{Python Code: Phase Portrait (Themed)}}
\scriptsize
\begin{lstlisting}
import numpy as np
import matplotlib.pyplot as plt
...
ax.plot(Vc, dVc_dt, color=curve)
ax.fill_between(Vc, dVc_dt, alpha=0.12)
ax.scatter([0], [125], color=accent)
ax.scatter([5], [0], color=accent)
